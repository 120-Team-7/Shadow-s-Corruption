class Practice extends Phaser.Scene {
    ...
    create() {
        ...
        this.text1 = this.add.text(centerX, screenHeight - this.textBoxY, "For the duration of the stun condition, enemies stop all actions and cannot contact damage. Each MELEE weapon stuns for 1 second. The MELEE knife has a 0.8 second cooldown. The MELEE orb has a 0.5 second cooldown before it can affect the same enemy. If you continuously stun an enemy, it is stun locked and unable to do anything.", practiceConfig).setOrigin(0.5, 0);
        this.text2 = this.add.text(centerX + screenWidth, screenHeight - this.textBoxY, "If the MELEE knife kills an enemy, you gain full CORRUPTION and the MELEE knife reload is instant rather than 0.8 seconds. Additionally, if your knife is corrupted, MELEE kills don't stop CORRUPTION ACTIVATION. In other words, a CORRUPTED MELEE knife can finish off many hurt enemies and still be shot as either a CORRUPTED orb or knife.", practiceConfig).setOrigin(0.5, 0);
        this.text2 = this.add.text(centerX + 2*screenWidth, screenHeight - this.textBoxY, "An enemy has two separate invulnerablity timers for when hit by the MELEE and shot orb that both last 0.5 seconds. By stunning an enemy with the MELEE orb and then walking into the enemy, you are able to shoot the orb on top of the enemy. Since the orb moves slowly at first and accelerates, it eventually catches up to the enemy to hit them again.", practiceConfig).setOrigin(0.5, 0);
        this.text3 = this.add.text(centerX + 3*screenWidth, screenHeight - this.textBoxY, "When you SHIFT, the MELEE weapon you held before you switched is dropped as a mine. The mine has all the same effects as the MELEE form of the weapon but rapidly disapates. The mine can be used as a momentary trap to protect you after SHIFTING.", practiceConfig).setOrigin(0.5, 0);
        this.text4 = this.add.text(centerX + 4*screenWidth, screenHeight - this.textBoxY, "CORRUPT attacks have bonus damage based on CORRUPTION level at the time of the attack. You gain CORRUPTION equal to the damage dealt of a knife attack. Using these facts, you can use the knife to chain CORRUPT knife throws and net 1 CORRUPTION each time due to the 1 base damage of the thrown knife.", practiceConfig).setOrigin(0.5, 0);
        this.text5 = this.add.text(centerX + 5*screenWidth, screenHeight - this.textBoxY, "Being efficient with CORRUPTION is key to doing well. A single CORRUPT attack always deal more overall damage than several normal attacks. Since each shot deals more damage, CORRUPTION attacks more decisively damages the ever-changing enemies. By repeatedly and quickly gaining CORRUPTION, SHIFTING, and using it, you can efficiently maintain your CORRUPTION at a high level.", practiceConfig).setOrigin(0.5, 0);
